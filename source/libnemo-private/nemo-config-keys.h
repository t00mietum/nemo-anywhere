...
static const NemoConfigKey nemo_config_keys[] = {
	...
	{ NEMO_ARCHIVE_COMMANDS_GROUP, NEMO_ARCHIVE_COMMAND_KEY_7Z, NEMO_CONFIG_STRING, NEMO_ARCHIVE_COMMAND_7Z_DEFAULT, NULL, NULL, "Command line 7-Zip is run with to create an archive. {{PROGRAM}} is the 7z it found, {{SOURCE_ITEMS}} the files and folders being packed, {{THREADS}} the share of the cores set under performance, and the rest stand for what the Compress dialog was asked for - drop one of those and that option stops reaching the program. Empty this line to get the built-in one back." },
	...
	{ "media-handling", "automount-open", NEMO_CONFIG_BOOL, "false", NULL, NULL, NULL },
	{ "performance", "cpu-percent", NEMO_CONFIG_INT, "50", NULL, NULL, "Percentage of the machine's CPU cores any compression may use. 50% is the best balance for hyperthreaded CPUs. Rounds up, so there is always at least one." },
	...
	{ "preferences", "always-show-tabs", NEMO_CONFIG_BOOL, "false", NULL, NULL, "Show the tab strip even when there is only one tab." },
	...
	{ "preferences", "detect-content", NEMO_CONFIG_BOOL, "false", NULL, NULL, "If true, enable detection of the type of content of a mounted media and display a suggested application to open the media." },
	...
	{ "preferences", "rename-selects-whole-name", NEMO_CONFIG_BOOL, "true", NULL, NULL, "Whether starting a rename selects the whole name. False selects only the part before the extension, which is what most file managers do." },
	...
	{ "preferences", "show-shortcut-extension", NEMO_CONFIG_BOOL, "false", NULL, NULL, "Whether a shortcut's .lnk or .desktop extension is part of the name shown" },
	...
	{ "preferences.menu-config", "selection-menu-extract", NEMO_CONFIG_BOOL, "true", NULL, NULL, "Show the selection context menu's Extract items." },

...
static void nemo_desktop_thumbnail_factory_init          (NemoDesktopThumbnailFactory      *factory);
static void nemo_desktop_thumbnail_factory_class_init    (NemoDesktopThumbnailFactoryClass *class);

G_DEFINE_TYPE_WITH_PRIVATE (NemoDesktopThumbnailFactory, nemo_desktop_thumbnail_factory, G_TYPE_OBJECT)
#define parent_class nemo_desktop_thumbnail_factory_parent_class
...
static void
nemo_desktop_thumbnail_factory_finalize (GObject *object)
{
  NemoDesktopThumbnailFactory *factory;
  NemoDesktopThumbnailFactoryPrivate *priv;
  
  factory = NEMO_DESKTOP_THUMBNAIL_FACTORY (object);
...
static void
nemo_desktop_thumbnail_factory_register_mime_types (NemoDesktopThumbnailFactory *factory,
                                                     Thumbnailer                  *thumb)
{
...
static void
nemo_desktop_thumbnail_factory_add_thumbnailer (NemoDesktopThumbnailFactory *factory,
                                                 Thumbnailer                  *thumb)
{
  NemoDesktopThumbnailFactoryPrivate *priv = factory->priv;
...
static void
update_or_create_thumbnailer (NemoDesktopThumbnailFactory *factory,
                              const gchar                  *path)
{
  NemoDesktopThumbnailFactoryPrivate *priv = factory->priv;
  GList *l;
  ...
  if (!found)
    {
      ...
      if (thumb)
        nemo_desktop_thumbnail_factory_add_thumbnailer (factory, thumb);
    }
...
static void
remove_thumbnailer (NemoDesktopThumbnailFactory *factory,
                    const gchar                  *path)
{
  NemoDesktopThumbnailFactoryPrivate *priv = factory->priv;
  GList *l;
...
static void
nemo_desktop_thumbnail_factory_load_thumbnailers (NemoDesktopThumbnailFactory *factory)
{
  NemoDesktopThumbnailFactoryPrivate *priv = factory->priv;
...
static void
nemo_desktop_thumbnail_factory_init (NemoDesktopThumbnailFactory *factory)
{
  NemoDesktopThumbnailFactoryPrivate *priv;
  
  factory->priv = nemo_desktop_thumbnail_factory_get_instance_private (factory);

  priv = factory->priv;
  priv->size = NEMO_DESKTOP_THUMBNAIL_SIZE_NORMAL;
  ...
  priv->permissions_problem = !nemo_desktop_thumbnail_cache_check_permissions (NULL, TRUE);
...
static void
nemo_desktop_thumbnail_factory_class_init (NemoDesktopThumbnailFactoryClass *class)
{
  GObjectClass *gobject_class;

  gobject_class = G_OBJECT_CLASS (class);
	
  gobject_class->finalize = nemo_desktop_thumbnail_factory_finalize;
}

/**
 * nemo_desktop_thumbnail_factory_new:
 * @size: The thumbnail size to use
 *
 * Creates a new #NemoDesktopThumbnailFactory.
 *
 ...
 * Since: 2.2
 **/
NemoDesktopThumbnailFactory *
nemo_desktop_thumbnail_factory_new (NemoDesktopThumbnailSize size)
{
  NemoDesktopThumbnailFactory *factory;
  
  factory = g_object_new (NEMO_DESKTOP_TYPE_THUMBNAIL_FACTORY, NULL);
...
/**
 * nemo_desktop_thumbnail_factory_lookup:
 ...
 **/
char *
nemo_desktop_thumbnail_factory_lookup (NemoDesktopThumbnailFactory *factory,
					const char            *uri,
					time_t                 mtime)
{
  NemoDesktopThumbnailFactoryPrivate *priv = factory->priv;
  ...
  path = g_build_filename (g_get_user_cache_dir (),
			   "thumbnails",
			   (priv->size == NEMO_DESKTOP_THUMBNAIL_SIZE_NORMAL)?"normal":"large",
			   file,
			   NULL);
  g_free (file);
  ...
  if (pixbuf != NULL)
    {
      res = nemo_desktop_thumbnail_is_valid (pixbuf, uri, mtime);
...
/**
 * nemo_desktop_thumbnail_factory_has_valid_failed_thumbnail:
 * @factory: a #NemoDesktopThumbnailFactory
 * @uri: the uri of a file
 * @mtime: the mtime of the file
 *
 ...
 **/
gboolean
nemo_desktop_thumbnail_factory_can_thumbnail (NemoDesktopThumbnailFactory *factory,
					       const char            *uri,
					       ...
					       time_t                 mtime)
{
  ...
  if (thumb || mimetype_supported_by_gdk_pixbuf (mime_type))
    {
      return !nemo_desktop_thumbnail_factory_has_valid_failed_thumbnail (factory,
                                                                          uri,
                                                                          mtime);
    }
 ...
 **/
GdkPixbuf *
nemo_desktop_thumbnail_factory_generate_thumbnail (NemoDesktopThumbnailFactory *factory,
						    const char            *uri,
						    const char            *mime_type)
{
  ...
  disabled = nemo_desktop_thumbnail_factory_is_disabled (factory, mime_type);
...
static void
maybe_fix_ownership (NemoDesktopThumbnailFactory *factory, const gchar *path)
{
...
static gboolean
make_thumbnail_dirs (NemoDesktopThumbnailFactory *factory)
{
  char *thumbnail_dir;
  char *image_dir;
  gboolean res;
  ...
  image_dir = g_build_filename (thumbnail_dir,
				(factory->priv->size == NEMO_DESKTOP_THUMBNAIL_SIZE_NORMAL)?"normal":"large",
				NULL);
...
static gboolean
make_thumbnail_fail_dirs (NemoDesktopThumbnailFactory *factory)
{
  char *thumbnail_dir;
  char *fail_dir;
  char *app_dir;
...
/**
 * nemo_desktop_thumbnail_factory_save_thumbnail:
 * @factory: a #NemoDesktopThumbnailFactory
 ...
 **/
void
nemo_desktop_thumbnail_factory_save_thumbnail (NemoDesktopThumbnailFactory *factory,
						GdkPixbuf             *thumbnail,
						const char            *uri,
						time_t                 original_mtime)
{
  ...
  if (tmp_fd == -1)
    {
      nemo_desktop_thumbnail_factory_create_failed_thumbnail (factory, uri, original_mtime);
      g_free (tmp_path);
      g_free (path);
      return;
    }
  ...
  if (width != NULL && height != NULL) 
    saved_ok  = gdk_pixbuf_save (thumbnail,
				 ...
				 "tEXt::Software", "Nemo Anywhere Thumbnail Factory",
				 NULL);
  else
    saved_ok  = gdk_pixbuf_save (thumbnail,
				 tmp_path,
...
/**
 * nemo_desktop_thumbnail_factory_create_failed_thumbnail:
 * @factory: a #NemoDesktopThumbnailFactory
 * @uri: the uri of a file
 ...
 **/
void
nemo_desktop_thumbnail_factory_create_failed_thumbnail (NemoDesktopThumbnailFactory *factory,
							 const char            *uri,
							 time_t                 mtime)
{
  char *path, *file;
 ...
 **/
char *
nemo_desktop_thumbnail_md5 (const char *uri)
{
...
/**
 * nemo_desktop_thumbnail_path_for_uri:
 * @uri: an uri
 ...
 **/
char *
nemo_desktop_thumbnail_path_for_uri (const char         *uri,
				      NemoDesktopThumbnailSize  size)
{
  char *md5;
  char *file;
  ...
  md5 = nemo_desktop_thumbnail_md5 (uri);
  file = g_strconcat (md5, ".png", NULL);
  g_free (md5);
  
  path = g_build_filename (g_get_user_cache_dir (),
			   "thumbnails",
			   (size == NEMO_DESKTOP_THUMBNAIL_SIZE_NORMAL)?"normal":"large",
			   file,
			   NULL);
...
/**
 * nemo_desktop_thumbnail_has_uri:
 ...
 **/
gboolean
nemo_desktop_thumbnail_has_uri (GdkPixbuf          *pixbuf,
				 const char         *uri)
{
...
/**
 * nemo_desktop_thumbnail_is_valid:
 ...
 **/
gboolean
nemo_desktop_thumbnail_is_valid (GdkPixbuf          *pixbuf,
				  const char         *uri,
				  time_t              mtime)
{
...
void
nemo_desktop_thumbnail_cache_fix_permissions (void)
{
    struct passwd *pwent;
 ...
 * @factory: (allow-none): an optional NemoDesktopThumbnailFactory
...
gboolean
nemo_desktop_thumbnail_cache_check_permissions (NemoDesktopThumbnailFactory *factory, gboolean quick)
{

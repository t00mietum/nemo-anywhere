...
- A shortcut always holds the absolute path, and Relative adds its path relative to itself, tried when the absolute one is gone, as Windows does. Made off Windows, the absolute path is the `\\server\share` one when the target is on a mounted share, else the path as that machine spells it. Windows 11 follows a shortcut only through its item ID list, which names the target in Windows' own terms, and no Linux path can be written that way. So Explorer cannot follow one, while Nemo Anywhere on Windows falls back to reading its paths itself.

...
- **The list view fills its width exactly.** No column is pushed off the end of the window and no strip of dead space is left after the last one. Widening, each column takes what it needs to show the longest value in it and then stops; Name is the only one with no such stop, so the rest is Name's. Narrowing, Name gives its surplus back first, then Type on its own down to about three characters, then everything together in proportion to how wide it is. A column whose values have no natural length - a type, a path, an owner - stops at a third of the Name column, so one long value cannot take the window. Widths follow what is actually in the folder rather than a guess made once: every row is measured as it arrives.
- **A width the user drags into place is remembered.** For a column with no natural length it becomes that column's ceiling for good, in settings: the column still follows its contents below it, but never grows past it again, however the window is resized. Name is the one column that never has a ceiling. Search results split the row differently - Name a third, Location the rest, and a dragged split is likewise kept for good. Type, left alone, defaults to at most twice the File extension column.
- **The column roster earns its defaults.** File extension shows by default between Name and Size, dot included the way Explorer's column shows it, and stays blank when the tail after a dot is not really an extension. Owner shows by default on Windows too, where the platform reports the file's real owner. Permissions source - whether a file's permissions are inherited from its folder, set on the file itself, or both - is offered on Windows, off by default.

...
- Make link asks what to make, one row for the folders and one for the files. Folders get a junction or a symlink on Windows and a symlink elsewhere. Files get a symlink or a hardlink. Either can be a `.lnk` shortcut, labeled Shortcut. A symlink path row picks relative or absolute for whatever comes out a symlink, and goes gray when nothing does, since a junction is always absolute and a hardlink has no path. A shortcut has its own row, below.
...
- A shortcut can hold several paths at once, and is followed by the first that still leads somewhere. So a shortcut gets checkboxes rather than a choice of one: Absolute, Relative and Portable, all checked to start with.

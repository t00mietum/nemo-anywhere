...
- **The list view fills its width exactly.** No column is pushed off the end of the window and no strip of dead space is left after the last one, and a small gap keeps the first and last of them off the window frame. Widening, each column takes what it needs to show the longest value in it and then stops; Name is the only one with no such stop, so the rest is Name's. With Location on the row the two grow together instead: they split whatever the other columns leave, Name takes no more than half, and so anything Name does not need for its longest name goes to Location. Narrowing, the growing columns give their surplus back first, then Type on its own down to about three characters, then Name, Location and Type together in proportion to how wide they are - a date or a size says nothing at all cut short, so those keep their width until the others are spent. A column whose values have no natural length - a type, an owner - stops at a third of what is growing and never ends up wider than Name or Location, so one long value cannot take the window. Widths follow what is actually in the folder rather than a guess made once: every row is measured as it arrives, and measured again when the zoom level changes the font or a column is switched on that was not there to measure.
- **A width the user drags into place is remembered.** For a column with no natural length it becomes that column's ceiling for good, in settings: the column still follows its contents below it, but never grows past it again, however the window is resized. Name is the one column that never has a ceiling, and Location has none either while it is growing alongside Name - dragging Location is what ends that and pins it. Search results size the pair differently: Name and Location take only as much as their contents need and the row ends short, and where the two do not both fit they give in proportion to what they asked for, neither ending more than twice the width of the other. Dragging either one pins a split of the row instead, and that is kept for good. Type, left alone, takes the width its longest value needs whenever the row has room, and gives it back first when the row runs short.

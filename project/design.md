...
- Installing is a script, not a package. The primary install path is a one-liner that fetches a release, checks it, and puts it where that platform expects - no repository to add, no dependency hunt, and no packaging format to maintain per distro. Distro packages can come later without changing this.
	...
	- Two scripts, one implementation of each platform's logic: `install.bash` for the unix side (bash 3.2, so macOS works untouched) and `install.ps1` for Windows. Run on Linux or macOS, the PowerShell script hands off to the bash one rather than repeating it.
...
- Install: `install.bash` (Linux, BSD, WSL, macOS) and `install.ps1` (Windows) at the repo root, run as one-liners straight from a shell. They read the releases page, so they depend on a fixed naming contract for release assets - the packaging stage has to produce exactly these names:

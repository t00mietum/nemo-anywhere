...
- Copying a link asks what should land at the far end. A link can be kept as a link or replaced by what it points at, and neither answer is right for every copy, so the question is put once per operation rather than guessed. It is only asked when the source actually holds links and the destination can hold at least one kind of them; a folder that keeps none, such as a FAT stick, gets a plain copy with nothing asked.
	...
	- Windows has two kinds of link where POSIX has one, and the dialog says so: a folder symlink and a junction both point at a folder, but only the symlink needs the privilege that Windows normally withholds. Each row starts on the kind it found, falls back to the nearest kind that still points at the same target, and falls back again to a copy. Anything the destination cannot take is greyed out rather than hidden, so the dialog does not change shape between machines.

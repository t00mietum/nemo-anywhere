...
- 🛠️ Randomly crashes. (At least on Windows, and before the multiple-process work.) Sometimes just with a focus change.
	...
	- Done: a crash now writes a report next to the settings file, under `crash/`. It carries the version, what killed it, and the stack. On Windows a minidump goes beside it and a dialog says where both went; everywhere else the same report also goes to stderr, which is what a launcher log keeps.

...
- ✅ Icon view:
	...
	- ✅ If folder is mostly images, increase default size to [max hieght or width = DPI-independent 320px].
		...
		- Done 20260920, third part: a folder remembers two icon sizes rather than one, and zooming sets whichever matches what is in the folder at the time. Both inherit, and a child picks between them by what is in the child, so one setting on a photo library gives every album under it the big size while a folder of notes in the same tree still opens small.

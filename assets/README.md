...
They exist because there is no cleanly-licensed SVG icon set in either style. What circulates is Microsoft's own shell art extracted and repackaged, sometimes under a license the repackager was not in a position to grant. This project will not ship that, so the two styles are drawn from scratch instead - a shared vocabulary of base shapes (folder, page, disc, drive, monitor, bin) and glyphs, recolored per theme.
